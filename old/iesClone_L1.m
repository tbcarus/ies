...
%Вводятся поток светодиода, мощность, их количества и размер светильника
%Перебираются все образцы в папке с m-фалом и сохраняются в папках по
%названиям. 
%Формат названия фалов "MD C40 A19 P1 83.ies"
...
N_LED =     [8       12       16      20]; %Количество светодиодов в создаваемом файле
dlina =     [0.628    0.928    1.228   1.528]; %длина светильника в создаваемом файле
...
%N_LED =     [9.49      12      15      21];
shirina =   [0.058  0.058  0.058  0.058  0.058]; %ширина светилньика в создаваемом файле
visota =    [0.063  0.063  0.063  0.063  0.063]; %высота светильника в создаваемом файле
...
for N_faila = 1:length(N_file)
    ...
    New_dir = 'Model One';
    mkdir(New_dir);
    lens_eff = str2num(files(N_file(N_faila)).name(name_pos(4):name_pos(4)+2))/100; %КПД линзы
    
%данные для ввода
power_LED = 0.98*0 + 0.74*1 + 1.04*1 + 1.02*1; %мощность одного СИД в рабочем режиме(без учёта ИП) WRGB
KLED = 1; %коэффициент для изменения потока 1 диода
stepA = 5; %шаг вывода азимутальных углов
stepP = 2; %шаг вывода полярных углов
FvLED = (141.5*0 + 49*1 + 88*1 + 19*1)*0.9*lens_eff; %световой поток одного светодиода, умноженный на светопропускание защитного стекла. КПД оптики берётся из переменной lens_eff
Power = 1; %Мощность двойная или одинарная
...
%for k=1:length(filename(:,1))
    ...
    cause = 1; %цикл считывания азимутальных углов. Аналогично полярным углам
    while (cause)
